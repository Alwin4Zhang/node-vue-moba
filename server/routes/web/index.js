module.exports = app => {
    ...
    return {
        name:li.innerText,
        heros:$$('li',$$('.hero-list')[i]).map(el => ({name:$$('h3',el)[0].innerHTML,avatar:$$('img',el)[0].src}))     
    }})
    */
        ...
        */
        ...
        cats.unshift({ //数组前面添加内容
            ...
            heroList: await Hero.find().where({
                categories: { $in: subCats }
            }).limit(10).lean() //populate categories关联出_id对应的categories信息
        })
        res.send(cats)
    })
    ...
    router.get('/heroes/:id', async (req, res) => {
        const data = await Hero
            ...
            .populate('categories items1 items2 partners.hero restraintHeros.hero restrainedHeros.hero') // 关联查询
            .lean()
        res.send(data)
    })
